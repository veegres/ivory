...
type Props = {
    ...
    setTarget: (target: PgCompactTableTarget) => void
}

export function PgCompactTableJobForm(props: Props) {
    const {node, cluster, target, onClick, setTarget} = props
    const [options, setOptions] = useState<PgCompactTableOptions>({force: false, noReindex: false, routineVacuum: false, initialReindex: false, noInitialVacuum: false})

    const start = useRouterPgCompactTableStart(cluster.name)
