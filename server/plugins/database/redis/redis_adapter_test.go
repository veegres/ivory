...
func TestConnectRequiresHostAndPort(t *testing.T) {
	tests := []struct {
		...
		{name: "missing host", ctx: database.Context{Connection: &database.Connection{Config: database.Config{Port: 6379}}}},
		{name: "missing port", ctx: database.Context{Connection: &database.Connection{Config: database.Config{Host: "localhost"}}}},
		{name: "placeholder host", ctx: database.Context{Connection: &database.Connection{Config: database.Config{Host: "-", Port: 6379}}}},
	}
