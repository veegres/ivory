...
// Test model for testing generic DbBucket operations
...
func TestBucket_Create(t *testing.T) {
	db := createTestDB(t)
	bucket := NewDbBucket[TestModel](db, "test-bucket")
...
func TestNewBucket_CreatesIfNotExists(t *testing.T) {
	...
	bucket := NewDbBucket[TestModel](db, "new-bucket")
...
func TestBucket_MultipleTypes(t *testing.T) {
	...
	bucket1 := NewDbBucket[TestModel](db, "bucket1")
	bucket2 := NewDbBucket[string](db, "bucket2")
