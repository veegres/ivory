...
func permissionRouter(g *gin.RouterGroup, rp *permission.Router, r *permission.Router) {
	...
	group.POST("/users/:permUsername/approve", rp.ValidateMethodMiddleware(coreConfig.ManagePermissionUpdate), r.ApproveUserPermission)
	group.POST("/users/:permUsername/reject", rp.ValidateMethodMiddleware(coreConfig.ManagePermissionUpdate), r.RejectUserPermission)
	group.DELETE("/users/:permUsername", rp.ValidateMethodMiddleware(coreConfig.ManagePermissionDelete), r.DeleteUserPermissions)
}
...
func nodeRouter(g *gin.RouterGroup, rp *permission.Router, r *node.Router) {
	...
	dbGroup.GET("/overview", rp.ValidateMethodMiddleware(coreConfig.ViewNodeDbOverview), r.GetNodeOverview)
	dbGroup.GET("/config", rp.ValidateMethodMiddleware(coreConfig.ViewNodeDbConfig), r.GetNodeConfig)
	dbGroup.PATCH("/config", rp.ValidateMethodMiddleware(coreConfig.ManageNodeDbConfigUpdate), r.PatchNodeConfig)
	dbGroup.POST("/switchover", rp.ValidateMethodMiddleware(coreConfig.ManageNodeDbSwitchover), r.PostNodeSwitchover)
	dbGroup.DELETE("/switchover", rp.ValidateMethodMiddleware(coreConfig.ManageNodeDbSwitchover), r.DeleteNodeSwitchover)
	dbGroup.POST("/reinitialize", rp.ValidateMethodMiddleware(coreConfig.ManageNodeDbReinitialize), r.PostNodeReinitialize)
	dbGroup.POST("/restart", rp.ValidateMethodMiddleware(coreConfig.ManageNodeDbRestart), r.PostNodeRestart)
	dbGroup.DELETE("/restart", rp.ValidateMethodMiddleware(coreConfig.ManageNodeDbRestart), r.DeleteNodeRestart)
	dbGroup.POST("/reload", rp.ValidateMethodMiddleware(coreConfig.ManageNodeDbReload), r.PostNodeReload)
	dbGroup.POST("/failover", rp.ValidateMethodMiddleware(coreConfig.ManageNodeDbFailover), r.PostNodeFailover)
	dbGroup.POST("/activate", rp.ValidateMethodMiddleware(coreConfig.ManageNodeDbActivation), r.PostNodeActivate)
