...
func NewManager(storage *files.Storage) *Manager {
	return &Manager{
		mu:      &sync.Mutex{},
...
func (s *Manager) Start(cmd Command) (JobID, error) {
	jobID := JobID(cmd.Id())
	s.mu.Lock()
	defer s.mu.Unlock()
...
// It handles START/END events, historical logs from files, and live updates.
func (s *Manager) Stream(id JobID, subID SubscriberID, send func(Message)) {
...
func (s *Manager) Stop(id JobID) error {
	s.mu.Lock()
	job, ok := s.jobs[id]
	s.mu.Unlock()
...
func (s *Manager) Subscribe(id JobID, subscriberID SubscriberID) (<-chan Message, error) {
	s.mu.Lock()
	job, ok := s.jobs[id]
	s.mu.Unlock()
...
func (s *Manager) Unsubscribe(id JobID, subscriberID SubscriberID) {
	s.mu.Lock()
	job, ok := s.jobs[id]
	s.mu.Unlock()
...
func (s *Manager) Status(id JobID) Status {
	s.mu.Lock()
	job, ok := s.jobs[id]
	s.mu.Unlock()
...
func (s *Manager) GetLogsPath(id JobID) (string, error) {
	if s.storage == nil {
...
func (s *Manager) unsafeAddJob(id JobID, job *Job) {
	s.jobs[id] = job
}

func (s *Manager) unsafeRemoveJob(id JobID) {
	delete(s.jobs, id)
}

func (s *Manager) removeJob(id JobID) {
	s.mu.Lock()
	defer s.mu.Unlock()
